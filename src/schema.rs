...
#[Object]
impl Query {
    async fn user_by_id<'ctx>(&self, ctx: &Context<'ctx>, id: Uuid, auth: Option<String>) -> Result<User> {
        let db = ctx.data::<DatabaseConnection>()?.clone();
        users::find_by_id(&db, id, Authorization::parse(auth)).await
    }

    async fn users<'ctx>(&self, ctx: &Context<'ctx>,
    ...
    async fn mods<'ctx>(&self, ctx: &Context<'ctx>,
        #[graphql(validator(maximum=10))] limit: Option<i32>,
        offset: Option<i32>,
